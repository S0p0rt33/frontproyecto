...
// Se mantiene la misma paleta para consistencia visual.
const appColors = {
  pageBg: '#121212',
...
const Registro = () => {
  ...
  const cardStyle = {
    ...
    padding: '40px 50px',
    width: '100%',
    maxWidth: '900px', // Tarjeta más ancha para el formulario de registro
  ...
  const titleStyle = { fontSize: '2.5rem', fontWeight: 'bold', color: appColors.textDark, marginBottom: '30px', textAlign: 'center' };
  ...
  const labelStyle = { display: 'block', color: appColors.textLight, marginBottom: '8px', fontSize: '0.9rem' };
  const inputStyle = {
    width: '100%', padding: '12px 15px', backgroundColor: 'rgba(0,0,0,0.2)',
    border: `1px solid ${appColors.borderColor}`, borderRadius: '8px', color: appColors.textDark,
    fontSize: '1rem', outline: 'none', transition: 'border-color 0.3s ease',
  };
  const buttonStyle = {
    gridColumn: '1 / -1', // Hace que el botón ocupe todo el ancho de la grilla
    width: '100%', padding: '14px', backgroundColor: appColors.primaryAccent, border: 'none',
  ...
  const errorStyle = { color: appColors.errorColor, fontSize: '0.8rem', minHeight: '18px', paddingTop: '4px' };
  ...
  if (isSuccess) {
    return (
      <div style={pageStyle}>
        <div style={cardStyle}>
          <h1 style={{...titleStyle, color: appColors.successColor}}>¡Registro Exitoso!</h1>
          <p style={{textAlign: 'center', color: appColors.textLight}}>
            Tu cuenta ha sido creada correctamente. Ahora puedes iniciar sesión con tus credenciales.
          </p>
           {/* Aquí podrías poner un Link de React Router para ir al login */}
           <button style={{...buttonStyle, maxWidth: '200px', margin: '30px auto 0 auto'}}>Ir a Iniciar Sesión</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={pageStyle}>
      <div style={cardStyle}>
        <h1 style={titleStyle}>Crear una <span style={{color: appColors.primaryAccent}}>Cuenta</span></h1>
